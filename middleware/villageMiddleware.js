...
const getVillagesByTaluka = (req, res, next) => {
  try {
    ...
    const villagesInTaluka = villageData.filter(
      (v) => v.taluka_code === talukaCode
    );
...
/**
 * Utility function to get villages by district code directly
 * Can be used without middleware
 *
 * @param {number|string} villageCode - The village code to look up
 * @returns {Object} Object containing district info and village codes
 */
const getVillagesByDistrictUtil = (villageCode) => {
  try {
    const villageCodeNum = parseInt(villageCode);
    ...
    // Get the district_code from the found village
    const districtCode = village.district_code;

    // Find all villages with the same district_code
    const villagesInDistrict = villageData.filter(
      (v) => v.district_code === districtCode
    );

    // Extract just the village codes
    const villageCodes = villagesInDistrict.map((v) => v.village_code);
    ...
    return {
      ...
      data: {
        ...
        district_name: village.district,
        district_name_marathi: village.district_marathi,
        total_villages: villagesInDistrict.length,
        village_codes: villageCodes,
